...
- **Selection-fields chip bar** — `UI.SelectionFields` becomes a row of clickable chips above `$filter`. Clicking a chip appends `<chip> eq ''` to `$filter` with the cursor parked inside the quotes. **Shift-click** instead appends the chip to `$select` (deduped). Chips for `RequiredProperties` render amber so you see at a glance which ones the server will reject queries without.
- **"⊞ filter bar" button** (next to the chip bar) — opens a mini Fiori-style filter form with one row per selection field: property label (+ type + `req` amber badge if required-in-filter), operator dropdown (`=`, `≠`, `>`, `≥`, `<`, `≤`, `contains`, `starts with`, `ends with`, `between`), value input (two inputs for `between`), and a per-row clear button. Apply writes an `and`-joined `$filter` expression into the query bar; literals are quoted per property `edm_type`. Cancel leaves the filter untouched; Reset clears the whole form.
