...
- **SAP View** — opt-in overlay that surfaces SAP/UI5 annotations on top of the raw data: `UI.HeaderInfo` titles, `Common.Text` / `sap:text` description pairings rendered per `UI.TextArrangement` (`Warehouse Berlin (WH01)` / `WH01 (Warehouse Berlin)` / text-only / separate columns), results-grid columns reordered to match `UI.LineItem` positions, `UI.Criticality` status colors, `Capabilities.*` and V2 `sap:filterable/sortable/creatable/updatable` restriction pills, `Measures.Unit` / `Measures.ISOCurrency` companions, `UI.SelectionFields` as clickable filter chips (amber when required-in-filter), a `UI.LineItem`-driven "Fiori cols" button that drops the default Fiori list-report columns into `$select` (augmented with `UI.PresentationVariant.RequestAtLeast` support fields so timezone/key-description columns come along automatically), a `UI.SelectionVariant`-driven "Fiori filter" button that rebuilds `$filter` from the declared default variant's Parameters + SelectOptions (SELECT-OPTIONS-style ranges), and `Common.ValueList` value-help pickers (F4): click the "⇒ F4" marker on a property to open a live picker against the value-help entity set, with picked rows bound back into `$filter` via the declared In/Out/InOut parameter mapping. Both inline `Common.ValueList` and the S/4HANA `Common.ValueListReferences` pattern (mapping lives in a separate F4 service) are supported — references resolve transparently on open. Pre-flight validator warns on queries that violate the declared restrictions without blocking the server from being the final judge.
